...
(function(scope) {
	...
	var useNative =
		!flags.register && hasNative && !window.ShadowDOMPolyfill && (!window.HTMLImports || HTMLImports.useNative);
	...
	if (useNative) {
		...
		function register(name, options) {
			...
			if (getRegisteredDefinition(name)) {
				throw new Error(
					"DuplicateDefinitionError: a type with name '" + String(name) + "' is already registered"
				);
			}
	...
	function _inserted(element) {
		...
		if (element.attachedCallback || element.detachedCallback || (element.__upgraded__ && logFlags.dom)) {
			...
			if (inDocument(element)) {
				...
				if (element.__inserted > 1) {
					logFlags.dom &&
						console.warn('inserted:', element.localName, 'insert/remove count:', element.__inserted);
	...
	function _removed(element) {
		...
		if (element.attachedCallback || element.detachedCallback || (element.__upgraded__ && logFlags.dom)) {
			...
			if (!inDocument(element)) {
				...
				if (element.__inserted < 0) {
					logFlags.dom &&
						console.warn('removed:', element.localName, 'insert/remove count:', element.__inserted);
...
(function() {
	...
	var win = window,
		...
		keypseudo = {
			action: function(pseudo, event) {
				return (
					pseudo.value.match(regexDigits).indexOf(String(event.keyCode)) > -1 == (pseudo.name == 'keypass') ||
					null
				);
			}
		},
    ...
    */
	...
	function syncAttr(element, attr, name, value, method) {
		var nodes = attr.property
				? [ element.xtag[attr.property] ]
				: attr.selector ? xtag.query(element, attr.selector) : [],
			index = nodes.length;
	...
	function attachProperties(tag, prop, z, accessor, attr, name) {
		...
		if (type == 'get') {
			...
			key[0] = prop;
			var setter = (tag.prototype[prop].set = xtag.applyPseudos(
				key.join(':'),
				attr
					? function(value) {
	...
	var xtag = {
		...
		register: function(name, options) {
			...
			if (tag.lifecycle.attributeChanged)
				tag.prototype.attributeChangedCallback = { value: tag.lifecycle.attributeChanged, enumerable: true };
					...
					if (condition)
						pseudo.source.condition = function(event, custom) {
							return xtag.query(this, pseudo.value).filter(function(node) {
								...
								? condition.call(this, event, custom)
								: null;
						};
				}
			},
			...
			if (content) {
				var div = frag.appendChild(doc.createElement('div')),
					nodes = toArray(
						content.nodeName
							? arguments
							: !(div.innerHTML = typeof content == 'function' ? parseMultiline(content) : content) ||
								div.children
					),
					length = nodes.length,
					index = 0;
        ...
        */
		...
		parseEvent: function(type, fn) {
			...
			event.chain =
				key +
				(event.pseudos.length ? ':' + event.pseudos : '') +
				(pseudos.length ? ':' + pseudos.join(':') : '');
				...
				return (
					...
					(this.__targetTouches__ =
						...
						(this.currentTarget == touchTarget ||
							(this.currentTarget.contains && this.currentTarget.contains(touchTarget)))
							? (this.identifier = 0) || [ this ]
							: [])
				);
			}
		},
